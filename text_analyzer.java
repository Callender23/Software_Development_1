...
public class ReadFile {
	...
	try {
		...
		HashMap<String, Integer> mappy = new HashMap<String, Integer>();	/* I used a Hashmap to store and sort my words here over other 
																			   methods due to its fast O(1) runtimes for many operation */
		...
		mappy.entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder()))  /* Here Im doing the sorting of the HashMap in a descending order. By using the
		 																							comparingByvalues(method) and calling the reverseOrder method(). After that
		 																							I use the forEachOrdered which performs an action, in this case the action is
		 																							putting the values of both keys and values to the LinkedHashMap we created in
		 																							a descending order*/	
